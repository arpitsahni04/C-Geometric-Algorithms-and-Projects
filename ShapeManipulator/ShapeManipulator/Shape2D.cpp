...
{
	// check if testpoint is inside BBox
	if ((testPnt.x > lowerBound.x) && (testPnt.x < upperBound.x)
		&& (testPnt.y > lowerBound.y) && (testPnt.y < upperBound.y)) {


		// cast rays from test point in any direction
		// horizontal for ease
		Point2D Endpnt = { upperBound.x + 500,testPnt.y };
		Point2D checkIntersect;
		int count = 0; // to count intersections
		Point2D AheadPnt;
		// check no of times ray instersects shape
		// loop through all sides of the shape 
		// and check if intersection is there 
		for (int i = 0; i < thePoints.size(); i++) {
			if (i == thePoints.size() - 1) {
				AheadPnt = thePoints[0];
			}
			else {
				AheadPnt = thePoints[i + 1];
			}
			checkIntersect = Line2D::getTrueIntersection(testPnt, Endpnt,
				thePoints[i], AheadPnt);
			if (checkIntersect.x != -INFINITY) {
				// Edge Case: Check if ray hits vertex or traces side of a shape
				// both points not above and not below testY
				// If intersection point.x  == of testX then on the edge
				if (checkIntersect.x == testPnt.x) {
					return isInclusive;
				}
				else if ((checkIntersect.y == thePoints[i].y) && AheadPnt.y >
					checkIntersect.y) {
					count++;
				}
				else if (checkIntersect.y == AheadPnt.y && thePoints[i].y >
					checkIntersect.y) {
					count++;
				}
				else {
	...
	// if odd number of times, test point is inside
	if (count % 2 != 0) {
		return true;
	}
